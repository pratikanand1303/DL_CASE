...
def sample(epoch, header, num_chars):
    ...
    for i in range(num_chars):
        batch = np.zeros((1, 1))
        if sampled:
            batch[0, 0] = sampled[-1] # creating a batch of only a value C_i which will store the next character 
        else:
            batch[0, 0] = np.random.randint(vocab_size) #if seed is given then use that and if it's not given and if it is the first element then use any random value from the input and generate a new character
